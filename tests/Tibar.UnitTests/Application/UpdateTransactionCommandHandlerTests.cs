...
public class UpdateTransactionCommandHandlerTests
{
    ...
    public UpdateTransactionCommandHandlerTests()
    {
        ...
        _handler = new UpdateTransactionCommandHandler(_contextMock.Object);
    }
    ...
    private static Mock<Microsoft.EntityFrameworkCore.DbSet<Category>> CreateMockCategories(Category category)
    {
        var mock = new Mock<Microsoft.EntityFrameworkCore.DbSet<Category>>();
        mock.Setup(m => m.FindAsync(It.IsAny<object[]>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(category);
        return mock;
    }

    private static Mock<Microsoft.EntityFrameworkCore.DbSet<Account>> CreateMockAccounts(Account account)
    {
    ...
    [Fact]
    public async Task Handle_NotInstallment_UpdatesOnlyTarget()
    {
        ...
        var other = new Transaction("Other", Money.Create(99), TransactionType.Expense, Guid.NewGuid(), Guid.NewGuid(), userId, new DateOnly(2026, 2, 1));

        var mockTransactions = CreateMockDbSet(new List<Transaction> { target, other });
        ...
        _contextMock.Setup(x => x.Categories).Returns(CreateMockCategories(category).Object);
        _contextMock.Setup(x => x.Accounts).Returns(CreateMockAccounts(account).Object);
        _contextMock.Setup(x => x.SaveChangesAsync(It.IsAny<CancellationToken>())).ReturnsAsync(1);

        var command = new UpdateTransactionCommand(target.Id, "New", 50, category.Id, account.Id, userId, new DateOnly(2026, 3, 1));
        var result = await _handler.Handle(command, CancellationToken.None);

        Assert.True(result.IsValid);
        Assert.Equal("New", target.Description);
        Assert.Equal(50, target.Amount.Amount);
        Assert.Equal(new DateOnly(2026, 3, 1), target.Date);
        Assert.Equal("Other", other.Description);
        Assert.Equal(99, other.Amount.Amount);
    }

    [Fact]
    public async Task Handle_WithInstallments_PropagatesSharedDataToGroup()
    {
        var userId = Guid.NewGuid();
        ...
        var mockTransactions = CreateMockDbSet(new List<Transaction> { target, member, other });
        mockTransactions.Setup(m => m.FindAsync(It.IsAny<object[]>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync(target);
        ...
        Assert.True(result.IsValid);
        Assert.Equal("New", target.Description);
        Assert.Equal("New", member.Description);
        Assert.Equal(50, target.Amount.Amount);
        Assert.Equal(50, member.Amount.Amount);
        Assert.Equal(category.Id, member.CategoryId);
        Assert.Equal(account.Id, member.AccountId);
        Assert.Equal("Other", other.Description);
        Assert.Equal(new DateOnly(2026, 3, 1), target.Date);
        Assert.Equal(new DateOnly(2026, 2, 1), member.Date);
    }
    ...
    public async Task Handle_NonExistentTransaction_ReturnsFailure()
    {
        ...
        var command = new UpdateTransactionCommand(Guid.NewGuid(), "New", 50, Guid.NewGuid(), Guid.NewGuid(), Guid.NewGuid(), new DateOnly(2026, 5, 27));
        var result = await _handler.Handle(command, CancellationToken.None);
